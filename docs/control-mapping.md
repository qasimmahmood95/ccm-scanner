...
| **IAM-16** | Authorization Mechanisms | `iam/no-wildcard-trust` | Role trust policies don't allow a wildcard principal unless a condition narrows *who* (`aws:PrincipalOrgID`, `sts:ExternalId`, …). A condition on something else (transport, region) leaves the principal open and is reported **NA**, not Pass. | `aws_iam_role.assume_role_policy` (`Principal` / `Principal.AWS` = `*`, `Condition`) | **FAIL** if principal is `*`/`AWS:*` with no `Condition`. | Yes |
