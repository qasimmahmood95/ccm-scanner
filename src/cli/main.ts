/**
 * The CLI's behaviour: the only place that reads the scanned input or writes
 * the evidence pack.
 *
 ...
 * and 2).
 *
 * This module has no side effects on import, so tests can drive `main` and
 * `runCli` directly. `index.ts` is the executable shim that calls it — kept
 * separate because deciding "am I the entry point?" from `process.argv[1]`
 * cannot be done reliably: npm installs the bin as a symlink, and Node
 * realpaths `import.meta.url` while leaving `argv[1]` as typed, so any such
 * guard makes the installed binary exit 0 without scanning.
 */
