...
import { createRegistry, domainOfCcmId } from "../../src/index.js";
import { IMPLEMENTED_DOMAINS, mapping } from "../support/mapping.js";
...
describe("the mapping table itself", () => {
  ...
  it("gives every row a recognised coverage class", () => {
    for (const row of mapping) {
      expect(["Yes", "Partial", "NA"], `row ${row.ccmId}`).toContain(row.coverage);
    }
  });
